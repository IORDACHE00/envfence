...
```ts
import * as e from "envfence";

export const config = e.env({
  PORT: e.port().default(3000).description("Port the server listens on"),
  DATABASE_URL: e.url().required().description("PostgreSQL connection string"),
  NODE_ENV: e.enumeration(["development", "production", "test"]).required(),
  SENTRY_DSN: e.url().requiredIf((v) => v.NODE_ENV === "production"),
  ENCRYPTION_KEY: e.string().required().minLength(32).maxLength(32),
...
- **`envalid`** validates but is verbose, and errors bury the lede.
- **`@t3-oss/env` / Zod-based**: powerful, but you're pulling Zod (~50KB) just to check `PORT=3000`.

`envfence` is the middle ground: a focused chainable API that reads like a schema, errors that tell you exactly what to fix and where, and full TypeScript inference — in ~2KB gzipped.

## Install

```sh
npm install envfence
```
...
import * as e from "envfence";
e.env({ PORT: e.port().default(3000) });
```

```ts
// Named (slightly less typing for small schemas)
import { env, port } from "envfence";
env({ PORT: port().default(3000) });
```

Both styles tree-shake correctly because `envfence` ships **individual named exports** (no runtime namespace object). Modern bundlers (esbuild, Rollup, Vite, Webpack 5, tsup) statically resolve `e.X` accesses and drop unused validators. If you only use `e.port` and `e.url`, the rest never make it into your bundle.

> **Why this matters:** Zod exports a single `z` object literal, which pins every validator into your bundle whether you use them or not. `envfence` deliberately avoids that shape so you get the namespace ergonomics *and* the tree-shaking. (You can name it anything: `import * as e`, `import * as ef`, `import * as env` — all work the same.)
...
e.env({
  ...
  WEBHOOK_SIGNING_SECRET: e.string().required().minLength(64).maxLength(64),

  // pattern + length, e.g. webhook secrets that start with whsec_
  STRIPE_WEBHOOK_SECRET: e.string()
    .required()
...
const config = e.env({
  NODE_ENV: e.enumeration(["development", "production"]),
  SENTRY_DSN: e.url().requiredIf((v) => v.NODE_ENV === "production"),
});
```
...
`envfence` infers everything from your schema. The `Infer<typeof schema>` helper is exported if you ever need the type explicitly:
...
const schema = {
  PORT: e.port().default(3000),
  API_KEY: e.string().required(),
};

type Config = e.Infer<typeof schema>;
// { PORT: number; API_KEY: string }

export const config = e.env(schema);
```
